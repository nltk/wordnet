...
def parse_wordnet_line(wordnet_line, parse_verb_frame=False):
    ...
    for symbol, pointer_offset, pointer_pos, lemma_ids_str in per_chunk(pointers_tokens, 4):
        if lemma_ids_str == '0000':
            ...
            lemma_pointers[source_lemma_name, symbol] = (pointer_pos, pointer_offset, target_index)
    ...
    synset_name = lemmas[0][0].lower()
    lemmas_objects = []
    # Creating the Lemma objects.
    for lemma in lemmas:
        lemma_name, lexname_index, lex_id, syn_mark = lemma
        lemmas_objects.append(Lemma(*lemma, synset_name=synset_name,
    ...
    offsets = _lemma_pos_offset_map[synset_name][pos]
    #print(offset, synset_name, pos, offsets)
    sense_index = offsets.index(offset)
    name = "%s/%s/%02i" % (synset_name, pos, sense_index+1)
    synset = Synset(offset, pos, name, lexname_index, synset_name,
                    definition, examples, synset_pointers, lemmas_objects)
